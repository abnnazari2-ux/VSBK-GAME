...
export default function GameScreen() {
  ...
  // Determine which player slot we occupy (0 = host, 1 = guest)
  ...
  const draw = useCallback(() => {
    ...
    // Use ref so aim line reflects current turn even inside rAF
    ...
    // Aim line + cue (only when it's this client's turn)
    const cb = getCueBall();
  ...
  // ── POST-SHOT LOGIC (ref avoids stale closure inside rAF loop) ─────────────
  ...
  // Show waiting indicator when opponent's turn
  useEffect(() => {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#050508', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      ...
      {/* Power slider — only on your turn */}
